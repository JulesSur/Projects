...
header('Access-Control-Allow-Methods: GET,POST,PUT,PATCH,DELETE,HEAD,OPTIONS');
    ...
    $sql= "SELECT * FROM Artikels WHERE source like 'De Standaard%' ORDER BY ammountID DESC LIMIT {$limit} OFFSET {$offset}";
    ...
    if(mysqli_num_rows($data)>0){
            while ($row = mysqli_fetch_array($data)){
                        echo '<a href="'. $row[link] .'" target="artikel"><div class="item">';
                            ...
                            echo '<div class="source" id="sourceDeStandaard">';
                                    if (strpos($row[source], 'De Standaard : Homepage') !== false) {
                                        echo '<img src="img/feedsLogos/StandaardLogo.png"><p>Hoofdpunten</p>';
                                    }
