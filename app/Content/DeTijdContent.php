...
header('Access-Control-Allow-Methods: GET,POST,PUT,PATCH,DELETE,HEAD,OPTIONS');
    ...
    $sql= "SELECT * FROM Artikels WHERE source like 'Tijd.be%' ORDER BY ammountID DESC LIMIT {$limit} OFFSET {$offset}";
    ...
    if(mysqli_num_rows($data)>0){
            while ($row = mysqli_fetch_array($data)){
                        echo '<a href="'. $row[link] .'" target="artikel"><div class="item">';
                            echo '<div class="source" id="sourceDeTijd">';
                                    if (strpos($row[source], 'Tijd.be Top stories') !== false) {
                                        echo '<img src="img/feedsLogos/DeTijdLogo.png"><p>Top stories</p>';
                                    }
